...
func sigmoid(x: float, _params={}):
	return 1 / (1 + exp(-1 * x))


func d_sigmoid(y: float, _params={}):
	return y * (1 - y)


func relu(x: float, _params={}):
	return 0.0 if x <= 0.0 else x


func d_relu(y: float, _params={}):
...
func d_leaky_relu(y: float, _params={}):
	return (0.01 if y < 0.0 else 1.0)


func tan_h(x: float, _params={}):
	return tanh(x)


func d_tan_h(y: float, _params={}):
	return 1 - pow(tanh(y), 2)


func arc_tan(x: float, _params={}):
	return atan(x)


func d_arc_tan(y: float, _params={}):
	return 1 / (pow(y, 2) + 1)


func bent_identity(x: float, _params={}):
	return ((sqrt(pow(x, 2) + 1) - 1) / 2) + x


func d_bent_identity(y: float, _params={}):
	return (y / (2 * sqrt(pow(y, 2) + 1))) + 1


func gaussian(x: float, _params={}):
	return exp(pow(-1 * x, 2))


func d_gaussian(y: float, _params={}):
